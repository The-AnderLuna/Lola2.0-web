...
export default function FlujoReserva() {
    ...
    // Scroll to top on step/category change instantly
    useEffect(() => {
        if (typeof window !== 'undefined') {
                                                            ...
                                                            <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-bg-card border border-border-subtle flex items-center justify-center shadow-[0_0_15px_rgba(212,175,55,0.1)] flex-shrink-0 relative overflow-hidden group">
                                                                ...
                                                                <Image 
                                                                    ...
                                                                    className={`w-[70%] h-[70%] object-contain drop-shadow-[0_0_8px_rgba(212,175,55,0.3)] transition-transform duration-300 ${
                                                                        ...
                                                                        categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('depilaci') ? 'scale-[2.0] md:scale-[2.1] group-hover:scale-[2.2]' :
                                                                        categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('medicina') ? 'scale-[1.95] md:scale-[2.05] group-hover:scale-[2.1]' :
                                                                        (categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('pesta') || categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('facial')) ? 'scale-[1.6] md:scale-[1.7] group-hover:scale-[1.75]' :
                                                                        categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('ceja') ? 'scale-[1.35] md:scale-[1.4] group-hover:scale-[1.45]' :
                                                                        categorias.find(c => c.id === activeCategory)!.nombre.toLowerCase().includes('labio') ? 'scale-[1.1] md:scale-[1.15] group-hover:scale-[1.2]' :
                                                                        'scale-[1.15] md:scale-[1.2] group-hover:scale-[1.25]'
