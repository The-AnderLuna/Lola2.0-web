...
export default function FlujoReserva() {
  ...
  useEffect(() => {
    if (categorias.length > 0 && typeof window !== "undefined") {
      ...
      if (preselectId || servicioParam) {
        ...
        if (foundService) {
          setSelectedServices([{ ...foundService, uid: crypto.randomUUID() }]);
          if (foundCatId) {
  ...
  const toggleService = (srv: Servicio) => {
    setSelectedServices(prev => {
      ...
      if (isSelected) {
        ...
        return [...prev, { ...srv, uid: crypto.randomUUID() }];
      }
    });
  ...
  const addServiceToCart = (srv: Servicio) => {
    setSelectedServices(prev => [...prev, { ...srv, uid: crypto.randomUUID() }]);
    setSelectedDate(null);
    setSelectedTime(null);
    setAvailableSlots([]);
  };
                          ...
                          <>
                            {Object.values(
                              ...
                              <div key={grp.id} className="bg-bg-surface border border-white/[0.06] rounded-2xl p-4 hover:border-gold/20 transition-all relative">
                                ...
                                <h4 className="text-sm font-bold text-text-primary leading-tight mb-1 pr-7">{grp.nombre}</h4>
                      ...
                      <div className="p-5 bg-bg-card border-t border-border-subtle space-y-4">
                        ...
                        {selectedServices.length >= 2 && (
                          <button
                            ...
                            className={`w-full py-3 rounded-xl border text-sm font-bold transition-all flex items-center justify-center gap-2 mb-3 ${
                              esReservaCompartida
                        ...
                        {esReservaCompartida && selectedServices.length >= 2 && (
                          <div className="mb-3 bg-bg-surface border border-gold/20 rounded-2xl p-4 space-y-3">
                            <p className="text-xs text-text-muted uppercase tracking-wider font-bold flex items-center gap-2">
                              <UserPlus className="w-3.5 h-3.5 text-gold" /> Servicios de tu amiga
                            </p>
                            <p className="text-[10px] text-text-muted leading-relaxed">Marca cuáles servicios son para ella:</p>
                                ...
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-all flex items-center justify-between ${
                                    isAmiga
                                      ...
                                      : 'bg-bg-base border border-border-subtle text-text-secondary hover:border-gold/30'
                                  }`}
                                >
                                  <span className="flex items-center gap-2">
                                    {isAmiga ? <CheckCircle className="w-3.5 h-3.5" /> : <Circle className="w-3.5 h-3.5" />}
                                    {srv.nombre}
                                  </span>
                                  <span>{formatCurrency(srv.precio)}</span>
                                </button>
                              );
                            })}
                            {serviciosAmiga.length > 0 && serviciosTitular.length === 0 && (
                              <p className="text-[10px] text-rose-400 font-semibold">⚠ Debes dejar al menos 1 servicio para ti.</p>
                            )}
                          </div>
                        )}
